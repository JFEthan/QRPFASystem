...
#			list: a b c
#
function(accu_parse_function_args)
...
macro( add_to_accu_include_dirs )
foreach( dir ${ARGN} )
  set_property( GLOBAL APPEND PROPERTY accu_INCLUDE_DIRS "${dir}" )
  endforeach()
endmacro()


macro( add_to_accu_libraries )
    foreach( lib ${ARGN} )
      ...
      if (TARGET ${lib})
          ...
          if (NOT ${is_lib_imported})
            set_property( GLOBAL APPEND PROPERTY accu_LIBRARIES "${lib}" )
          else()
            # For imported targets, we just want to depend on the library directly
            get_target_property(libpath ${lib} LOCATION)
            if (libpath)
                set_property( GLOBAL APPEND PROPERTY accu_LIBRARIES "${libpath}" )
...
macro( add_to_accu_sources )
    ...
    foreach(_src ${ARGN})
        if(_relPath)
            set_property( GLOBAL APPEND PROPERTY accu_SOURCES "${_relPath}/${_src}" )
        else()
            set_property( GLOBAL APPEND PROPERTY accu_SOURCES "${_src}" )
        endif()
    endforeach()
endmacro()

macro( add_to_accu_headers )
    if("${SDK_SOURCE_DIR}" STREQUAL "")
        file(RELATIVE_PATH _relPath "${CMAKE_SOURCE_DIR}" "${CMAKE_CURRENT_SOURCE_DIR}")
    else()
        file(RELATIVE_PATH _relPath "${SDK_SOURCE_DIR}" "${CMAKE_CURRENT_SOURCE_DIR}")
    ...
    foreach(_hdr ${ARGN})
        if(_relPath)
            set_property( GLOBAL APPEND PROPERTY accu_HEADERS "${_relPath}/${_hdr}" )
        else()
            set_property( GLOBAL APPEND PROPERTY accu_HEADERS "${_hdr}" )
        endif()
    endforeach()
endmacro()

macro( accu_set_compile_flags file flags )
...
#=============================================================================
#
#	accu_strip_optimization
#
function(accu_strip_optimization name)
	set(_compile_flags)
...
#
#	Usage:
#		accu_join(OUT ${OUT} [ LIST ${LIST} ] GLUE ${GLUE})
#
#	Input:
#		LIST		: list to join
...
#
#	Example:
#		accu_join(OUT test_join LIST a b c GLUE ";")
...
#=============================================================================
function(accu_add_module)

        accu_parse_function_args(
                NAME accu_add_module
                ONE_VALUE MODULE
        ...
        accu_add_library(${MODULE} STATIC EXCLUDE_FROM_ALL ${SRCS})

        # Pass variable to the parent accu_add_module.
        ...
        foreach(prop LINK_FLAGS)
                if(${prop})
                        accu_join(OUT ${prop} LIST ${${prop}} GLUE " ")
                endif()
        endforeach()

        if(COMPILE_FLAGS AND ${_no_optimization_for_target})
             accu_strip_optimization(COMPILE_FLAGS ${COMPILE_FLAGS})
        endif()
        foreach (prop LINK_FLAGS )
           if (${prop})
	...
	set_property(GLOBAL APPEND PROPERTY accu_LIBRARIES ${MODULE})
        set_property(GLOBAL APPEND PROPERTY accu_MODULE_PATHS ${CMAKE_CURRENT_SOURCE_DIR})
endfunction()

#=============================================================================
#
#	accu_add_optimization_flags_for_target
#
set(all_posix_cmake_targets "" CACHE INTERNAL "All cmake targets for which optimization can be suppressed")
function(accu_add_optimization_flags_for_target target)
	...
	# Pass variable to the parent accu_add_library.
	set(_no_optimization_for_target ${_no_optimization_for_target} PARENT_SCOPE)
endfunction()
...
#
function(accu_add_executable target)
	add_executable(${target} ${ARGN})
	accu_add_optimization_flags_for_target(${target})
endfunction()
...
#
function(accu_add_library target)
	...
	set_property(GLOBAL APPEND PROPERTY accu_LIBRARIES ${target})
        set_property(GLOBAL APPEND PROPERTY accu_MODULE_PATHS ${CMAKE_CURRENT_SOURCE_DIR})
...
#
#	Example:
#		accu_prebuild_targets(OUT target_list BOARD r16)
#
function(accu_prebuild_targets)
        accu_parse_function_args(
                        NAME accu_prebuild_targets
                        ONE_VALUE OUT BOARD
                        REQUIRED OUT
                        ARGN ${ARGN})
