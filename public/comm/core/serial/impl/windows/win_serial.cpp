...
size_t Serial::SerialImpl::write(const uint8_t *data, size_t length) {
	//printf_s("SerialImpl::write send 1111 \n" );
  if (is_open_ == false) {
    return 0;
  }
  //printf_s("SerialImpl::write send 2222 \n");
  if (data == NULL || length == 0) {
    return 0;
  }

  DWORD    error;
 // printf_s("SerialImpl::write send 3333 \n");
  ...
  if (!::WriteFile(fd_, data, static_cast<DWORD>(length), &bytes_written,
     ...
     printf_s("Serial::SerialImpl::write 3333 errCode:%d bytes_written:%d fd_:%0x hEvent:%0x\n", dwError,bytes_written,fd_,writeCompletionOverlapped.hEvent);
    if (GetLastError() == ERROR_IO_PENDING) {
		while (!GetOverlappedResult(fd_, &writeCompletionOverlapped, &bytes_written, TRUE)) {

			dwError = GetLastError();
			if (dwError == ERROR_IO_INCOMPLETE) {

				dwBytesSent += bytes_written; 
				continue;
			}
			else if (dwError == 31)
			{
      ...
      /*  DWORD dwWait =::WaitForSingleObject(fd_, INFINITE);
        ...
        if (dwWait != WAIT_OBJECT_0) {
          if (!GetOverlappedResult(fd_, NULL, &bytes_written, TRUE)) {
            if (GetLastError() != ERROR_IO_INCOMPLETE) {
              int errCode = GetLastError(); 
                printf_s("Serial::SerialImpl::write 2222 errCode:%d\n", errCode); 
              return 0;
            }
          }
        }*/
